    ...
    if(user_id, date, time) {
        try {
            ...
            if(schedule_check[0].length > 0) {
                ...
                const number = phoneNumber[0][0]

                await db.promise().query(`INSERT INTO appointment(user_id, date, time, contact_number, status) VALUES('${user_id}', '${date}', '${time}', '${phoneNumber[0][0]}', '${'Waiting'}' ) `)
                res.status(201).send({msg:"Schedule Added!", contact_number:`${number}`})
            }
        }
        catch(err) {
            console.log(err)
